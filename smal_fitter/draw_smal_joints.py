        ...
        for image_sgl, landmarks_sgl, visible_sgl in zip(image_np, landmarks_np, visible_np):
            ...
            for joint_id, ((y_co, x_co), vis) in enumerate(zip(landmarks_sgl, visible_sgl)):
                ...
                if not vis:
                    ...
                    inv_ctr += 1
                    continue
                cv2.drawMarker(image_sgl, (x_co, y_co), (int(color[0]), int(color[1]), int(color[2])),
                               marker_type, 8, thickness=3)
                cv2.putText(image_sgl, f"{config.PIG_MODEL_JOINTS_NAME[joint_id]}", (x_co, y_co),
                            cv2.FONT_HERSHEY_COMPLEX, 1, (int(color[0]), int(color[1]), int(color[2])))
